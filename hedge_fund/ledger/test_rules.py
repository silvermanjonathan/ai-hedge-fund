...
def test_consensus_long_fires_and_does_not():
    assert (
        consensus_long(verdicts(v("a", "bullish", 60), v("b", "bullish", 70), v("c", "bullish", 55)), CFG)
        == "3 schools bullish at >= 55, none bearish"
    )
    assert (
        consensus_long(verdicts(v("a", "bullish", 60), v("b", "bullish", 70), v("c", "bullish", 54)), CFG) is None
    )  # third below min_conf
    assert (
        consensus_long(
            verdicts(v("a", "bullish", 60), v("b", "bullish", 70), v("c", "bullish", 80), v("d", "bearish", 20)), CFG
        )
        is None
    )  # any bear blocks
...
def test_consensus_short_mirrors():
    assert (
        consensus_short(verdicts(v("a", "bearish", 60), v("b", "bearish", 70), v("c", "bearish", 55)), CFG) is not None
    )
    assert (
        consensus_short(
            verdicts(v("a", "bearish", 60), v("b", "bearish", 70), v("c", "bearish", 55), v("d", "bullish", 10)), CFG
        )
        is None
    )
...
def test_contrarian_follows_the_named_school_only():
    ...
    assert (
        contrarian(verdicts(v("schloss", "bullish", 90)), PlaybookConfig(follow="schloss", follow_conf=80))
        == "schloss bullish at 90"
    )


def test_resilience_confirmed_is_described_as_balance_sheet():
    reason = resilience_confirmed(
        verdicts(v("dalio_resilience", "bullish", 60), v("a", "bullish", 58), v("b", "bullish", 70)), CFG
    )
    assert reason == "resilient balance sheet + 2 schools bullish"
    assert "macro" not in reason
    assert (
        resilience_confirmed(verdicts(v("dalio_resilience", "bullish", 60), v("a", "bullish", 58)), CFG) is None
    )  # needs two others
    assert (
        resilience_confirmed(
            verdicts(v("dalio_resilience", "neutral", 60), v("a", "bullish", 58), v("b", "bullish", 70)), CFG
        )
        is None
    )
    assert (
...
def test_candidates_sorted_and_header_exact(tmp_path):
    ...
    assert [c.ticker for c in cands] == [
        ...
        "THREE",
        "ONE",
    ]  # two rules first, then one rule ordered by mean confidence (70 > 65)
